...
async function testApi() {
  try {
    const email = 'audit_products@nora.com';
    const pwd = 'audit123';
    ...
    console.log('User created. Logging into Vercel...');
    ...
    // 3. Call API
    const apiRes = await fetch(url + '/api/admin/products', { headers: { Cookie: cookie } });
    console.log('API Status:', apiRes.status);
    const data = await apiRes.json();
    console.log('Product Count Returned:', data.data ? data.data.length : data);
